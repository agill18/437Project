...
function loginUser(req, res) {
  ...
  if (!email || !password) {
    ...
    import_credentials.default.verify(email, password).then((email2) => generateAccessToken(email2)).then((token) => res.status(200).send({ token })).catch((error) => res.status(401).send("Unauthorized"));
  }
}
;
